...
class Category
{
    ...
    function deleteCategory($id = null)
    {
        ...
        $extraCategory = $this->getExtraCategory($id);
        if(count($extraCategory) == 0) return FALSE;

        //=======================================================
        $parentFolderNode = $this->CI->folder->_getNodeById($extraCategory[0]['folder_id']);

        $extraParentFolder = $this->CI->folder->getExtraFolder($parentFolderNode);

        $this->CI->folder->modifyExtraFolder(array(
            $this->CI->folder->sort_column_name => str_replace('c'.$id.';','',$extraParentFolder[$this->CI->folder->sort_column_name])
        ), $parentFolderNode);
