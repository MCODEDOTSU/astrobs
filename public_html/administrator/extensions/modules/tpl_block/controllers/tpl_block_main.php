...
class Tpl_block_main extends Admin_Controller
{
    var $moduleName = 'tpl_block';
    var $tables;

    function Tpl_block_main()
    {
    ...
    function index()
    {
        $Main = $this->db->from($this->tables['tpl_block'])->where('side', 'main')->get()->result_array();

        $this->module->parse($this->moduleName, 'tpl_block_main.php', array(
            'form_open' => form_open('admin/tpl_block/tpl_block_main/save'),
    ...
    function save()
    {
        ...
        $this->db->delete($this->tables['tpl_block'], array('side' => 'main'));
        $this->db->insert($this->tables['tpl_block'], array('title' => $title,'body' => $body, 'side'=>'main'));

        redirect('admin/tpl_block/tpl_block_main');
    }
}
?>
