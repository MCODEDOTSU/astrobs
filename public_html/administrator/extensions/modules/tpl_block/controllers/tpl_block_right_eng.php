...
class Tpl_block_right_eng extends Admin_Controller
{
    var $moduleName = 'tpl_block';
    var $tables;

    function Tpl_block_right_eng()
    {
    ...
    function index()
    {
        $Right = $this->db->from($this->tables['tpl_block'])->where('side', 'right_eng')->get()->result_array();

        $this->module->parse($this->moduleName, 'tpl_block_right.php', array(
            'form_open' => form_open('admin/tpl_block/tpl_block_right_eng/save'),
    ...
    function save()
    {
        ...
        $this->db->delete($this->tables['tpl_block'], array('side' => 'right_eng'));
        $this->db->insert($this->tables['tpl_block'], array('title' => $title, 'body' => $body, 'side'=>'right_eng'));

        redirect('admin/tpl_block/tpl_block_right_eng');
    }
}
