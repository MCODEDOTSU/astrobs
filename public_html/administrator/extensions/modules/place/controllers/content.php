...
//    unlink('.uploads/photo/'.$news[0]['file_name']); 
            ...
            default:
                ...
                foreach($this->module->config($type, 'place') as $file => $functions)
                {
                	if ($type == 'news') {
                    	...
                    	$this->$file->$functions['create'](array('file_id'=>$result, 'lang_id' => $this->_getRootDir ($result)));
                    } else {
	...
	function _getRootDir ($id) {
		$file = $this->db->where ('id', $id)->limit (1)->get ('th_file')->result_array ();

		if ($file[0]['folder_id'] > 0) {
			$fold = $this->db->where ('id', $file[0]['folder_id'])->get ('th_folder')->result_array ();
		}
		else if ($file[0]['category_id'] > 0) {
			$cat = $this->db->select ('folder_id')->where ('id', $file[0]['category_id'])->get ('th_category')->result_array ();
			$fold = $this->db->where ('id', $cat[0]['folder_id'])->get ('th_folder')->result_array ();
		}

		while (true) {
			$q = $this->db->like ('sort', 'f' . $fold[0]['id'] . ';')->get ('th_folder')->result_array ();
			if (count ($q) > 0){
				$fold = $q;
			} else {
				break;
		...
		$q = $this->db->select ('id')->where ('fid', $fold[0]['id'])->get ('th_language_structure')->result_array ();
		return $q[0]['id'];
	}
