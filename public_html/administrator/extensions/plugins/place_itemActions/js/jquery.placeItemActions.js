(function ($) {
    ...
    $.fn.placeItemActions = function (settings) {
        $.placeItemActions.previous(this);
    }

    $.extend($.placeItemActions, {

        settiяяяяяяяяяяяяngs: {
            item: 'div.place_item',
            attr: 'item',
            container: 'li',
        ...
        previous: function (e) {
            ...
            $(e).each(function () {
                ...
                $(container).hover(
                    function () {
                        $(".item_actions", $(this)).show();
                    },
                    function () {
                        $(".item_actions", $(this)).hide();
                    }
                );
                ...
                if (dataArray[0] == 'f') {
                    $('<a href="#" title="Редактировать">' + $.placeItemActions.settings.icon.edit + '</a>').appendTo($(div)).click(function () {
                        $.placeItemActions.edit(dataArray);
                        return false;
                    });
                }
                $('<a href="#" title="Переименовать">' + $.placeItemActions.settings.icon.rename + '</a>').appendTo($(div)).click(function () {
                    $.placeItemActions.rename(dataArray, item, container);
                    return false;
                });
                $('<a href="#" title="Удалить">' + $.placeItemActions.settings.icon.remove + '</a>').appendTo($(div)).click(function () {
                    $.placeItemActions.remove(dataArray, container);
                    return false;
                });
