<?php
    ...
    if(!in_array($wh, $allowed_sizes)){
    	die('Ќедопустимый размер изображени€!');
    }
    $src=$_dir.'/'.$file.'.'.$_type;
    if(!file_exists($src)){
    ...
    if(!file_exists($cache) || filemtime($cache)<filemtime($src)){
    	...
    	$_type = strtr($_type, 'ABCDEFGHIJKLMNOPQRSTUVWXYZјЅ¬√ƒ≈®∆«»… ЋћЌќѕ–—“”‘’÷„ЎўЏџ№Ёёя', 'abcdefghijklmnopqrstuvwxyzабвгдеЄжзийклмнопрстуфхцчшщъыьэю€');
	    switch($_type){
	    case 'jpg':
	    case 'jpeg':
            ...
            if($w>=1024 || $h>=1024){
                die('—лишком большой размер картинки');
            }
	...
	function imageSmartResize($imSrc,$dstW,$dstH,$method=2,$bkgColorHex='FFFFFF') {
	    ...
	    method=2 - уменьшать картинку, добавл€ть белое поле
